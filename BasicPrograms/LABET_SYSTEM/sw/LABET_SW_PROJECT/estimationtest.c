...
int main()
{
	...
	if(pthread_create(&thread, NULL, readDataThread, NULL) == 0){
	...
	*/
	...
	while (1)
	{
		...
		//getCorrections(sigCorPtr, curPosAlt, curPosLon, curPosLat, theta, phi, curPosYaw, targetDir, targetPosAlt, targetPosLat, targetPosLon, targetPitch, targetRoll);
		/**
		...
		//printf("\nPitch: %i Roll: %i",RCValuesArr[1],RCValuesArr[2]);
...
void getEstimatedInclination(){
  ...
  if (firstSample){
    ...
    theta = -atan(RwEst[0]/sqrt(squared(RwEst[1]) + squared(RwEst[2])));
    phi =atan(RwEst[1]/sqrt(squared(RwEst[0]) + squared(RwEst[2])));
    yaw = atan(RwEst[2]/sqrt(squared(RwEst[0]) + squared(RwEst[1])));
